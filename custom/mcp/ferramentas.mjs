...
export const FERRAMENTAS = [
  ...
  criar('filas', 'staydesk/queues', 'queue', 'Cria uma fila de encaminhamento.', {
    ...
    reoffer_same_agent: BOOLEANO,
    reoffer_after_seconds: NUMERO,
    load_queue_keys: LISTA,
    channel_types: LISTA,
    inbox_ids: LISTA,
    conditions: LISTA,
  }),
  ...
  listar('filas_de_carga', 'staydesk/load_queues', 'Canais de trabalho: que canal e que caixa contam como chat e como ticket.'),
  ...
  listar('regras_de_capacidade', 'staydesk/capacity_rules', 'Regras de capacidade: quantas conversas de cada canal de trabalho o agente aguenta ao mesmo tempo, a padrão e as atribuídas.'),
  criar('regras_de_capacidade', 'staydesk/capacity_rules', 'capacity_rule', 'Cria uma regra de capacidade.', {
    name: TEXTO,
    description: TEXTO,
    limits: OBJETO,
    is_default: BOOLEANO,
  ...
  listar('status_do_agente', 'staydesk/agent_statuses', 'Status de disponibilidade do agente: o que cada um recebe (chat, ticket) e o atual de quem chama.'),
